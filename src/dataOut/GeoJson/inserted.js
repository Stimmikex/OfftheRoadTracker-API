...
export const getGeoData = async (geojsonFilePath) => {
    try {
        ...
        geojsonData.features.forEach(async (el) => {
            const areaId = "SELECT area_id FROM zone WHERE name = $1";
            const areaArr = [el.properties.area]
            const typeId = "SELECT id AS type_id FROM type WHERE name = $1";
            const typeArr = ["Point"]
            const getAreaId = await query(areaId, areaArr)
            const getTypeId = await query(typeId, typeArr)

            const insertTrack = "INSERT INTO track (name, date, length, area_id, description, year) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *";
            const trackArr = [
              el.properties.name,
              el.properties.date,
              el.properties.length,
              getAreaId.rows[0].area_id,
